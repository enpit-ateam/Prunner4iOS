...
class TableView: UITableViewCell {
  ...
  func setCell(history: History) {
    ...
    dayLabel.text = day.description + "日"
    timeLabel.text = String(format: "%d:%2d", DayService.getComponent(date: startDate).hour!, DayService.getComponent(date: startDate).minute!)
    distanceLabel.text = String(format: "%.3f", distance)
    runTimeLabel.text = String(format: "%d:%2d分", hour, minute)
